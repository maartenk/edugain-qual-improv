...
def get_database_file_path() -> Path:
    """Get database file path, honoring override environment variables."""
    env_file = os.getenv("EDUGAIN_WEB_DB_FILE")
    if env_file:
        db_path = Path(env_file).expanduser()
        db_path.parent.mkdir(parents=True, exist_ok=True)
        return db_path

    cache_base = os.getenv("EDUGAIN_WEB_CACHE_DIR")
    if cache_base:
...
def get_database_path() -> str:
    """Get database path as SQLAlchemy URL."""
    env_url = os.getenv("EDUGAIN_WEB_DB_URL")
    if env_url:
        return env_url
