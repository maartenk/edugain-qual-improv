...
./scripts/dev-env.sh --fresh --with-tests --with-web   # Full stack: pytest/ruff/pre-commit/cov/xdist + FastAPI/SQLAlchemy
...
# Generate HTML coverage report (requires the [coverage] extra)
pytest --cov=src/edugain_analysis --cov-report=html
open htmlcov/index.html

# Parallel execution with xdist (requires the [parallel] extra)
pytest -n auto
```

**Coverage:** When the `[coverage]` extra is installed, the suite reports ~82% overall (279 tests: 98-100% for CLI, 89-96% for core modules, 62-67% for integration-level web modules). See [COVERAGE_ANALYSIS.md](COVERAGE_ANALYSIS.md) for detailed breakdown and future improvement ideas.

### Coverage Configuration
- **HTML reports**: Generated in `htmlcov/` when running with `pytest --cov`
- **XML reports**: Enable with `--cov-report=xml` when needed (optional)
- **Multi-version testing**: Python 3.12, 3.13, and 3.14 tracked separately in CI
- **Parallel coverage**: Supported by installing the `[parallel]` extra alongside `[coverage]`
...
- **Continue on error**: Only used for non-critical validation steps; tests now fail-fast for clarity
